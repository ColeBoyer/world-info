...
def world():
    projects = [
        {"name": "project 1", "desc": "the first project"},
        {"name": "project 2", "desc": "the second project"},
    ]
...
def create_world():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f"Congratz, {world.name} created!")
...
def view_world(world_id):
    ...
    projects = db.session.scalars(
        db.select(Project)
        .where(Project.world_id == world_id)
...
def user(username):
    ...
    worlds = db.session.scalars(
        sa.select(World).where(World.user_id == current_user.get_id())
    )
